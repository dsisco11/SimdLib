...
class AbiRegister final
{
  public:
	...
	SIMDLIB_ABI_NOINLINE AbiRegister SIMD_FLAGS(InOut, RegisterOnly) simdlib_abi_unary(this AbiRegister value) noexcept
	{
		return AbiRegister{api_type::bitwise_not(value.m_data)};
	}

	/** @brief Mirrors a binary explicit-object member boundary. */
	SIMDLIB_ABI_NOINLINE AbiRegister SIMD_FLAGS(InOut, RegisterOnly) simdlib_abi_binary(this AbiRegister lhs, AbiRegister rhs) noexcept
	{
		return AbiRegister{api_type::add(lhs.m_data, rhs.m_data)};
	}

	/** @brief Mirrors a ternary explicit-object member boundary. */
	SIMDLIB_ABI_NOINLINE AbiRegister SIMD_FLAGS(InOut, RegisterOnly) simdlib_abi_ternary(this AbiRegister lhs, AbiRegister rhs, AbiRegister addend) noexcept
	{
	...
	SIMDLIB_ABI_NOINLINE std::uint32_t SIMD_FLAGS(In, RegisterOnly) simdlib_abi_scalar(this AbiRegister value) noexcept
	{
		return api_type::movemask(value.m_data);
	}

	/** @brief Mirrors a register-shaped mask-result explicit-object member boundary. */
	SIMDLIB_ABI_NOINLINE AbiMask SIMD_FLAGS(InOut, RegisterOnly) simdlib_abi_mask(this AbiRegister value) noexcept
	{
		(void)value;
	...
	SIMDLIB_ABI_NOINLINE native_type SIMD_FLAGS(InOut, RegisterOnly) simdlib_abi_native(this AbiRegister value) noexcept
	{
		return value.m_data;
	}

	/** @brief Mirrors a store explicit-object member boundary. */
	SIMDLIB_ABI_NOINLINE void SIMD_FLAGS(In) simdlib_abi_store(this AbiRegister value, float *destination) noexcept
	{
	...
	SIMDLIB_ABI_NOINLINE auto SIMD_FLAGS(In) simdlib_abi_mutate(this AbiRegister &lhs, AbiRegister rhs) noexcept -> AbiRegister &
	{
...
SIMDLIB_ABI_NOINLINE register_type SIMD_FLAGS(InOut, RegisterOnly) simdlib_consumer_abi_register_return(register_type lhs, register_type rhs) noexcept
{
...
SIMDLIB_ABI_NOINLINE native_type SIMD_FLAGS(InOut, RegisterOnly) simdlib_consumer_abi_register_pass(register_type value) noexcept
{
	return value.native;
}

/** @brief Returns a real RegisterMask across a separately compiled ABI boundary. */
SIMDLIB_ABI_NOINLINE mask_type SIMD_FLAGS(In, RegisterOnly) simdlib_consumer_abi_mask_return(register_type lhs, register_type rhs) noexcept
{
	return lhs.compare_equal(rhs);
}

/** @brief Passes a real RegisterMask across a separately compiled ABI boundary. */
SIMDLIB_ABI_NOINLINE native_type SIMD_FLAGS(Out, RegisterOnly) simdlib_consumer_abi_mask_pass(mask_type value) noexcept
{
	return value.native;
}

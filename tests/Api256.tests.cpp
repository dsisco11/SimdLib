...
TEST_CASE("256-bit runtime extraction covers every lane and element type", "[simdlib][avx2][extract][runtime]")
{
...
{
	using bytes = SimdLib::Api<256, std::uint8_t>;
	REQUIRE(bytes::to_array(bytes::set1(0x81)) == std::array<std::uint8_t, 32>{0x81, 0x81, 0x81, 0x81, 0x81, 0x81, 0x81, 0x81, 0x81, 0x81, 0x81,
	...
	REQUIRE(bytes::to_array(bytes::multiply(bytes::set1(0x81), bytes::set1(2))) ==
			std::array<std::uint8_t, 32>{2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2});

...
class uint128_t final
{
  private:
	...
	template <class> struct simd_element
	{
		using type = std::uint64_t;
	};

	/** @brief Internal facade used whenever the 128-bit SIMD backend is available. */
	template <class Dependency = void> using simd = Api<128, typename simd_element<Dependency>::type>;

	template <class Dependency = void> inline static constexpr bool simd_available = is_api_available_v<128, typename simd_element<Dependency>::type>;
  ...
  public:
	...
	constexpr uint128_t(const std::uint64_t lower, const std::uint64_t upper) noexcept : m_data{lower, upper} {}

	template <std::integral T> constexpr uint128_t(const T value) noexcept : m_data{static_cast<std::uint64_t>(value), 0} {}

	constexpr uint128_t(const bool value) noexcept : m_data{static_cast<std::uint64_t>(value), 0} {}
	...
	{
		return uint128_t(static_cast<std::uint64_t>(simd<Dependency>::template extract<0>(value)),
	...
	[[deprecated("Prefer SimdLib::Bmi::bextr")]] [[nodiscard]] constexpr uint128_t extract(const std::uint8_t len, const std::uint8_t start) const noexcept
	{
		if (len == 0 || start >= 128)
		{
			return {};
	...
	template <std::size_t len>
		requires(len <= 64)
	[[deprecated("Prefer SimdLib::Bmi::bextr")]] [[nodiscard]] constexpr std::uint64_t extract(const std::uint8_t start) const noexcept
	{
	...
	[[deprecated("Prefer SimdLib::Bmi::bextr")]] [[nodiscard]] constexpr uint128_t extract() const noexcept
	{
	...
	template <int width> [[nodiscard]] static constexpr uint128_t create_mask(const int offset) noexcept
	{
	...
	template <std::integral T> constexpr uint128_t &operator<<=(const T count) noexcept
	{
	...
	[[nodiscard]] constexpr std::uint64_t &low() noexcept
	{
		return m_data[0];
	}
	[[nodiscard]] constexpr std::uint64_t &high() noexcept
	{
		return m_data[1];
	}
	...
	template <std::integral T> [[nodiscard]] constexpr explicit operator T() const noexcept
	{
  ...
  private:
	...
	[[nodiscard]] static constexpr add_carry_result portable_add_with_carry(const std::uint64_t lhs, const std::uint64_t rhs,
	...
	[[nodiscard]] static constexpr subtract_borrow_result portable_subtract_with_borrow(const std::uint64_t lhs, const std::uint64_t rhs,
	...
	[[nodiscard]] static constexpr add_carry_result add_with_carry(const std::uint64_t lhs, const std::uint64_t rhs, const bool carryIn = false) noexcept
	{
#if SIMDLIB_USE_COMPILER_CARRY_INTRINSICS && SIMDLIB_COMPILER_MSVC && defined(_M_X64)
		if (!std::is_constant_evaluated())
		{
			std::uint64_t result = 0;
			const unsigned char carry = _addcarry_u64(static_cast<unsigned char>(carryIn), lhs, rhs, &result);
		...
		if (!std::is_constant_evaluated())
		{
			...
			const bool secondCarry = __builtin_add_overflow(partial, static_cast<std::uint64_t>(carryIn), &result);
	...
	[[nodiscard]] static constexpr subtract_borrow_result subtract_with_borrow(const std::uint64_t lhs, const std::uint64_t rhs,
																			   const bool borrowIn = false) noexcept
	{
#if SIMDLIB_USE_COMPILER_CARRY_INTRINSICS && SIMDLIB_COMPILER_MSVC && defined(_M_X64)
		if (!std::is_constant_evaluated())
		{
			std::uint64_t result = 0;
			const unsigned char borrow = _subborrow_u64(static_cast<unsigned char>(borrowIn), lhs, rhs, &result);
		...
		if (!std::is_constant_evaluated())
		{
			...
			const bool secondBorrow = __builtin_sub_overflow(partial, static_cast<std::uint64_t>(borrowIn), &result);
	...
	template <std::integral T> [[nodiscard]] static constexpr int normalize_shift(const T count) noexcept
	{
	...
	constexpr void shift_left(const int count) noexcept
	{
		...
		m_data = {m_data[0] << count, (m_data[1] << count) | (m_data[0] >> (64 - count))};
	}
	...
	constexpr void shift_right(const int count) noexcept
	{
		...
		m_data = {(m_data[0] >> count) | (m_data[1] << (64 - count)), m_data[1] >> count};
	}
...
template <> class numeric_limits<SimdLib::uint128_t> : public numeric_limits<std::uint64_t>
{
  public:
	...
	[[nodiscard]] static constexpr SimdLib::uint128_t epsilon() noexcept
	{
		return {};
	}
	[[nodiscard]] static constexpr SimdLib::uint128_t round_error() noexcept
	{
		return {};
	}
	[[nodiscard]] static constexpr SimdLib::uint128_t infinity() noexcept
	{
		return {};
	}
	[[nodiscard]] static constexpr SimdLib::uint128_t quiet_NaN() noexcept
	{
		return {};
	}
	[[nodiscard]] static constexpr SimdLib::uint128_t signaling_NaN() noexcept
	{
		return {};
	}
	[[nodiscard]] static constexpr SimdLib::uint128_t denorm_min() noexcept
	{
		return {};
	}
};

template <> struct hash<SimdLib::uint128_t>
{
...
{
	return value.low() == std::numeric_limits<std::uint64_t>::max() ? 64 + std::countr_one(value.high()) : std::countr_one(value.low());
}
...
{
	return value.high() == std::numeric_limits<std::uint64_t>::max() ? 64 + std::countl_one(value.low()) : std::countl_one(value.high());
}

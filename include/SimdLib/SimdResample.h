...
inline void ReduceBytesToBitsBy8_Parity(const std::span<const std::uint8_t> src, const std::span<std::uint8_t> dst) noexcept
{
	...
	const auto parityLut =
		U8x16::setr(std::uint8_t{0}, std::uint8_t{1}, std::uint8_t{1}, std::uint8_t{0}, std::uint8_t{1}, std::uint8_t{0}, std::uint8_t{0}, std::uint8_t{1},
					std::uint8_t{1}, std::uint8_t{0}, std::uint8_t{0}, std::uint8_t{1}, std::uint8_t{0}, std::uint8_t{1}, std::uint8_t{1}, std::uint8_t{0});
...
inline void ExpandBitsToBytesBy8(const std::span<const std::uint8_t> src, const std::span<std::uint8_t> dst) noexcept
{
	...
	const auto zero = U8x16::setzero();
	const auto allOnes = U8x16::set1(std::uint8_t{0xFF});
	const auto laneMasks = U8x16::setr(std::uint8_t{1}, std::uint8_t{2}, std::uint8_t{4}, std::uint8_t{8}, std::uint8_t{16}, std::uint8_t{32}, std::uint8_t{64},
									   std::uint8_t{0x80}, std::uint8_t{1}, std::uint8_t{2}, std::uint8_t{4}, std::uint8_t{8}, std::uint8_t{16},
									   std::uint8_t{32}, std::uint8_t{64}, std::uint8_t{0x80});
	const auto expandPair = [&](const std::uint8_t low, const std::uint8_t high) noexcept
	{
		const auto bits = U8x16::setr(low, low, low, low, low, low, low, low, high, high, high, high, high, high, high, high);
	...
	for (std::size_t pair = 0; pair < pairCount; ++pair)
		U8x16::store_unaligned(expandPair(src[pair * 2], src[pair * 2 + 1]), std::span<std::uint8_t, 16>(dst.data() + pair * 16, 16));
	if ((src.size() & 1u) != 0)

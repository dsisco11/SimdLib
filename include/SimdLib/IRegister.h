...
/** @brief Reports whether a Register-shaped type exposes zero initialization. */
template <class register_t>
concept Zero = Type<register_t> && requires {
	{ register_t::zero() } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes scalar broadcast construction. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type accepts the supplied logical lane list. */
...
/** @brief Reports whether a Register-shaped type exposes fixed-size array construction. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes logical-extent unaligned loading. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes logical-extent aligned loading. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes logical-extent raw-byte loading. */
template <class register_t>
concept LoadBytes = Type<register_t> && requires(std::span<const std::byte, register_t::lane_count * sizeof(typename register_t::element_type)> source) {
	{ register_t::load_bytes(source) } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes logical-extent unaligned storage. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes logical-extent aligned storage. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes logical-extent raw-byte storage. */
template <class register_t>
concept StoreBytes =
	Type<register_t> && requires(register_t value, std::span<std::byte, register_t::lane_count * sizeof(typename register_t::element_type)> destination) {
		{ value.store_bytes(destination) } -> std::same_as<void>;
	};

/** @brief Reports whether a Register-shaped type exposes fixed-size array conversion. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes one compile-time-selected lane. */
...
/** @brief Reports whether a Register-shaped type can replace one compile-time-selected lane. */
template <class register_t, std::size_t index>
...
/** @brief Audits the invariant-bearing construction, transfer, and observation surface shared by Register-shaped types. */
template <class register_t>
concept CoreSurface = Type<register_t> && Zero<register_t> && Broadcast<register_t> && FromArray<register_t> && Load<register_t> && LoadAligned<register_t> &&
					  LoadBytes<register_t> && Store<register_t> && StoreAligned<register_t> && StoreBytes<register_t> && ToArray<register_t> &&
					  Lane<register_t, 0> && WithLane<register_t, 0>;
...
/** @brief Reports whether a Register-shaped type exposes register subtraction. */
template <class register_t>
concept Subtract = Type<register_t> && requires(register_t lhs, register_t rhs) {
	{ lhs - rhs } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes register multiplication. */
template <class register_t>
concept Multiply = Type<register_t> && requires(register_t lhs, register_t rhs) {
	{ lhs * rhs } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes register division. */
template <class register_t>
concept Divide = Type<register_t> && requires(register_t lhs, register_t rhs) {
	{ lhs / rhs } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes register remainder. */
template <class register_t>
concept Modulus = Type<register_t> && requires(register_t lhs, register_t rhs) {
	{ lhs % rhs } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes arithmetic negation. */
template <class register_t>
concept Negate = Type<register_t> && requires(register_t value) {
	{ -value } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes lane-wise minimum. */
template <class register_t>
concept Min = Type<register_t> && requires(register_t value) {
	{ value.min(value) } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes lane-wise maximum. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes adjacent multiply-add for the requested source type. */
...
/** @brief Reports whether a Register-shaped type exposes unsigned/signed byte multiply-add for the requested source type. */
template <class register_t, class source_element_t = typename register_t::element_type>
...
/** @brief Reports whether a Register-shaped type exposes byte sum-of-absolute-differences for the requested source type. */
template <class register_t, class source_element_t = typename register_t::element_type>
...
/** @brief Reports whether a Register-shaped type exposes immediate-controlled multi-SAD for the requested source type. */
...
/** @brief Reports whether a Register-shaped type exposes saturating horizontal subtraction. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes alternating add-subtract. */
template <class register_t>
concept AddSubtract = Type<register_t> && requires(register_t value) {
	{ value.add_subtract(value) } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes an immediate-controlled dot product. */
...
/** @brief Reports whether a Register-shaped type exposes bitwise AND. */
template <class register_t>
concept BitwiseAnd = Type<register_t> && requires(register_t lhs, register_t rhs) {
	{ lhs & rhs } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes bitwise OR. */
template <class register_t>
concept BitwiseOr = Type<register_t> && requires(register_t lhs, register_t rhs) {
	{ lhs | rhs } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes bitwise XOR. */
template <class register_t>
concept BitwiseXor = Type<register_t> && requires(register_t lhs, register_t rhs) {
	{ lhs ^ rhs } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes bitwise complement. */
template <class register_t>
concept BitwiseNot = Type<register_t> && requires(register_t value) {
	{ ~value } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes bitwise AND-NOT. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes one sign bit per logical lane. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes per-lane left shift. */
template <class register_t>
concept ShiftLeft = Type<register_t> && requires(register_t value) {
	{ value << 1 } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes logical per-lane right shift. */
template <class register_t>
concept LogicalShiftRight = Type<register_t> && requires(register_t value) {
	{ value.logical_shift_right(1) } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes signedness-selected per-lane right shift. */
template <class register_t>
concept ShiftRight = Type<register_t> && requires(register_t value) {
	{ value >> 1 } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes explicit slow-path logical-payload dynamic byte left shift. */
template <class register_t>
concept ShiftBytesLeftSlow = Type<register_t> && requires(register_t value) {
	{ value.shift_bytes_left_slow(1) } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes explicit slow-path logical-payload dynamic byte right shift. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes immediate logical-payload byte left shift. */
...
/** @brief Reports whether a Register-shaped type exposes immediate logical-payload byte right shift. */
template <class register_t, int count>
...
/** @brief Reports whether a Register-shaped type exposes explicit slow-path logical-payload dynamic bit left shift. */
template <class register_t>
concept ShiftBitsLeftSlow = Type<register_t> && requires(register_t value) {
	{ value.shift_bits_left_slow(1) } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes explicit slow-path logical-payload dynamic bit right shift. */
template <class register_t>
concept ShiftBitsRightSlow = Type<register_t> && requires(register_t value) {
	{ value.shift_bits_right_slow(1) } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes logical-payload compile-time bit left shift. */
template <class register_t, int count>
...
/** @brief Reports whether a Register-shaped type exposes logical-payload compile-time bit right shift. */
template <class register_t, int count>
...
/** @brief Reports whether a Register-shaped type exposes ordered equality comparison. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes ordered greater-than comparison. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes ordered greater-than-or-equal comparison. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes ordered less-than comparison. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes ordered less-than-or-equal comparison. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes logical-register equality. */
template <class register_t>
concept Equal = Type<register_t> && requires(register_t lhs, register_t rhs) {
	{ lhs == rhs } -> std::same_as<bool>;
};

/** @brief Reports whether a Register-shaped type exposes logical-register inequality. */
template <class register_t>
...
/** @brief Reports whether a Register-shaped type exposes an immediate-controlled low-half shuffle. */
template <class register_t, int immediate>
concept ShuffleLow = Type<register_t> && requires(register_t value) {
	{ value.template shuffle_low<immediate>() } -> std::same_as<register_t>;
};

/** @brief Reports whether a Register-shaped type exposes an immediate-controlled high-half shuffle. */
template <class register_t, int immediate>
...
/** @brief Reports whether a Register-shaped type exposes an immediate-controlled two-register blend. */
template <class register_t, int immediate>
...
/** @brief Reports whether a Register-shaped type can reinterpret its logical bit pattern as the requested element type. */
...
/** @brief Reports whether a Register-shaped type can numerically convert every lane to the requested element type. */
template <class register_t, class target_t>
...
/** @brief Reports whether a Register-shaped type can widen its lowest lanes into the requested target register. */

...
concept PartialRegisterAvailable = RegisterAvailable<element_t, bits> && (active_lane_count > 0) &&
	(active_lane_count < Api<bits, element_t>::element_count) &&
...
using NativePartialRegister = PartialRegister<
	element_t, (PartialRegisterAvailable<element_t, 256, active_lane_count> ? 256 : 128), active_lane_count>;

namespace Detail
{
/** @brief Selects a partial or complete result without forming a constraint-invalid unselected specialization. */
template <class element_t, std::size_t bits, std::size_t result_lane_count, bool use_complete_register>
struct PartialOrCompleteRegisterSelector;
...
struct PartialOrCompleteRegisterSelector<element_t, bits, result_lane_count, true>
{
...
template <class element_t, std::size_t bits, std::size_t result_lane_count>
struct PartialOrCompleteRegisterSelector<element_t, bits, result_lane_count, false>
{
...
using partial_or_complete_register_t = typename Detail::PartialOrCompleteRegisterSelector<
	element_t, bits, result_lane_count,
	result_lane_count == Api<bits, element_t>::element_count || !PartialRegisterAvailable<element_t, bits, result_lane_count>>::type;
...
template <class element_t, std::size_t bits, std::size_t active_lane_count>
	requires PartialRegisterAvailable<element_t, bits, active_lane_count> && std::is_integral_v<element_t> &&
		IApi::MultiplyAddAdjacent<Api<bits, element_t>>
using partial_multiply_add_adjacent_result_t = partial_or_complete_register_t<
	partial_multiply_add_adjacent_element_t<element_t>, bits, (active_lane_count + 1) / 2>;
...
using partial_byte_multiply_add_result_t =
	partial_or_complete_register_t<std::int16_t, bits, (active_lane_count * sizeof(element_t) + 1) / 2>;
...
template <class element_t, std::size_t bits, std::size_t active_lane_count>
	requires PartialRegisterAvailable<element_t, bits, active_lane_count> && std::is_integral_v<element_t> &&
		IApi::MagnitudeChecked<Api<bits, element_t>>

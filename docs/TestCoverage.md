...
| `Bmi::bextr(uint128_t)` | The length/start and encoded-control overloads use the same table to directly cover zero length, out-of-range starts, cross-word extraction, ordinary extraction, truncation at the upper object boundary, and ignored high control bits. |

...
| `bextr` overloads | Supported | Runtime controls use `(source, length, start)`. The control-mask overload reads start from bits 0-7 and length from bits 8-15 while ignoring higher bits. Zero length or start at/above width returns zero; ranges truncate at source width. Template controls are compile-time constrained to the intrinsic control field. | Exhaustive 8-bit and randomized 32/64-bit tests; constexpr probes |

...
| `pp_xor`, `ps_xor`, `pp_and`, `ps_and`, `pp_nand`, `ps_nand`, `pp_nandi`, `ps_nandi` | Supported, contract clarification required | The current implementations are distance-1 adjacent-bit transforms of the form `value op (value >> 1)` or `value op (value << 1)`. They do not compute a complete all-prefix scan across the word. The XOR/AND names and Doxygen descriptions therefore require reconciliation with the implemented adjacent-bit contract; NAND and inverse-NAND additionally require an explicit evaluation definition because NAND is not associative. | Exhaustive 8-bit scalar oracles preserve the current adjacent-bit results pending that clarification |
...
promotion defects in the NAND and unset/trailing-mask helper families. The
